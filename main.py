...
def draw_text(surface, value, pos, size, color, center=False, bold=False):
    ...
    rect = image.get_rect(center=pos if center else None)
    if not center:
        rect.topleft = pos
...
def glow(surface, center, color, radius, alpha=70):
    ...
    for step in range(4, 0, -1):
        pygame.draw.circle(layer, (*color, alpha * step // 8), (middle, middle), radius * step // 4)
...
def land(position, previous_bottom, velocity_y, width, height, platforms):
    ...
    candidates = [platform for platform in platforms if current.right > platform.left and current.left < platform.right and previous_bottom <= platform.top + 4 and current.bottom >= platform.top]
...
class Wave:
    ...
    def draw(self, surface, camera):
        ...
        pygame.draw.ellipse(layer, (*self.color, alpha), (int(self.position.x - camera - self.radius), int(self.position.y - self.radius * .45), int(self.radius * 2), int(self.radius * .9)), self.width)
...
class Hazard:
    ...
    def draw(self, surface, camera, phase):
        ...
        if not self.armed:
            ...
            pygame.draw.circle(surface, (255, 221, 125), (x, y), self.radius + 11 + pulse, 2)
            pygame.draw.line(surface, (255, 190, 90), (x - 20, y), (x + 20, y), 2)
            return
        glow(surface, (x, y), danger, self.radius * 3, 65)
        if self.kind == "ring":
            pygame.draw.circle(surface, danger, (x, y), self.radius + int(self.age * 32), 7)
            pygame.draw.circle(surface, (255, 240, 190), (x, y), max(3, self.radius // 3), 2)
        else:
...
class Player:
    ...
    def attack(self, particles):
        ...
        for _ in range(8):
            particles.append(Particle(Vector2(self.attack_rect.center), Vector2(random.uniform(40, 150) * self.facing, random.uniform(-40, 40)), (220, 245, 255), 3, .28, 0, 3))
        return True
    ...
    def damage(self, amount, particles):
        ...
        for _ in range(18):
            particles.append(Particle(Vector2(self.rect.center), Vector2(random.uniform(-180, 180), random.uniform(-180, 40)), (255, 100, 135), 4, .55, 320))
        return True
        ...
        if self.attack_cooldown <= 0:
            ...
            if self.phase == 1:
                ...
                if self.attack_serial % 2:
                    ...
                    hazards.append(Hazard(Vector2(player.position.x, GROUND_Y - 38), Vector2(), 32, 18, "ring", .48, 1.8))
...
class Game:
    ...
    def update(self, dt):
        ...
        if self.state == "intro":
            ...
            self.pressure = .3
            if random.random() < min(1, dt * 12): self.burst(self.boss.rect.center, (125, 55, 190), 3, True)
        ...
        if self.boss.attack_timer > 0 and self.boss.attack_timer > .35:
        ...
        for hazard in self.hazards:
            if hazard.armed and hazard.position.distance_to(self.player.rect.center) < hazard.radius + 25:
                if self.player.damage(hazard.damage, self.particles): hazard.hit = True
        ...
        if self.player.health < previous_health or self.boss.health < previous_boss:
            self.damage_flash = 1.0
            self.screen_shake = max(self.screen_shake, .22)
    ...
    def draw(self):
        ...
        if self.boss.phase == 2 and self.boss.transition > 0:
            ...
            scene.blit(overlay, (0, 0))
            draw_text(scene, "A ABISMO DESPERTA", (WIDTH // 2, 130), 31, (255, 230, 180), center=True, bold=True)
